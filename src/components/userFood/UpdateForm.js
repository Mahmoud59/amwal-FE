...
const UpdateForm = () => {
    ...
    useEffect(() => {
        axios.get(`http://127.0.0.1:8000/api/v1/users/${decodedAccessKey['uuid']}/foods/${id}/`, headers)
        .then(res => {
            setSelectedMalOption(res.data.meal);
            setSelectedFood(res.data.food.name);
            setCalorieValue(res.data.calorie_value);
            setDoseDateTime(new Date(res.data.dose_time).toISOString().substring(0, 16));
        })
        .catch(err => {
            console.log(err);
        });
    ...
    const handleSubmit = (event) => {
        ...
        axios.patch(`http://127.0.0.1:8000/api/v1/users/${decodedAccessKey['uuid']}/foods/${id}/`, {
